...
#spec = [('m',int32),('n',int32),('kappa',float64),('dist',float64),('tot_power',float64),('points',float64[:]),('top_type',float64[:]),('bot_type',float64[:]),('other_blocks',optional(test_type))]
...
a = block.block(np.ones((height_a,width_a))*30,150,p,dist)
b = block.block(np.ones((height_b,width_b))*30,230,0,dist)
c = block.block(np.ones((height_c,width_c))*30,248,0,dist)

list_of_fins = []
for i in range(number_f):
    list_of_fins.append(block.block(np.ones((height_f,width_f))*30,248,0,dist))
